// Function to call the API to fetch data and display it on the home page
function getCharacters() {
    ...
    const char_Url = `http://gateway.marvel.com/v1/public/characters?&ts=${ts}&apikey=${publicKey}&hash=${hash}&limit=100`;
    getCharApi(char_Url);
  }
  ...
  searchbar.onkeyup = function (event) {
    ...
    if (userData != '') {
      while (ul.hasChildNodes()) {
        ...
        const ts = Date.now().toString();
        const hash = CryptoJS.MD5(ts + privateKey + publicKey).toString();
        const api_Url = `http://gateway.marvel.com/v1/public/characters?nameStartsWith=${userData}&ts=${ts}&apikey=${publicKey}&hash=${hash}`;
        getApi(api_Url);
    }
                        ...
                        `
                            <a href=${"/character.html?character=" + data[i].id}git class="flex j-sb a-c" style="text-decoration:none;color:black;">
                                <div class="left flex a-c">
                                    <img src="${data[i].thumbnail.path}.${data[i].thumbnail.extension}" alt="" id="img1">
